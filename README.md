...
SCI2是一个先进的科学计算项目，专注于流体动力学仿真和物理信息神经网络（PINN）研究。项目集成了多种深度学习架构，包括MLP、LSTM、GRU、Transformer等，并支持对比学习和对抗学习增强预测器，为科学计算提供强大的AI驱动解决方案。
...
# 测试对抗学习模块
python src/adversarial_learning/test_adversarial.py
```
...
│       ├── config.yaml      # 对抗学习配置
│       ├── adversarial_models.py # 对抗学习模型
│       ├── adversarial_trainer.py # 对抗学习训练器
│       ├── adversarial_losses.py # 对抗学习损失函数
│       ├── adversarial_augmentation.py # 对抗数据增强
│       ├── main.py          # 对抗学习主程序
│       └── test_adversarial.py # 对抗学习测试
├── scripts/                  # 测试脚本
│   └── test_main.py         # 主测试脚本
└── data/                     # 数据文件
...
### 对抗学习配置 (`src/adversarial_learning/config.yaml`)

对抗学习模块的专用配置：

- **AdversarialModel**: 对抗学习模型参数（生成器、判别器、预测器）
- **AdversarialTrain**: 对抗学习训练参数（三阶段训练）
- **AdversarialAttack**: 对抗攻击配置（FGSM、PGD、CW）
- **Augmentation**: 对抗数据增强策略
- **AdversarialLoss**: 对抗学习损失函数配置
...
对抗学习模块特性：
- **三阶段训练**: 预训练 → 对抗训练 → 微调
- **GAN架构**: 生成器、判别器、预测器协同训练
- **多种攻击**: 支持FGSM、PGD、CW等对抗攻击
- **鲁棒性增强**: 提升模型对对抗样本的鲁棒性
- **物理约束**: 集成物理信息神经网络约束

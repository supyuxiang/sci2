...
# 添加散度损失权重参数
parser.add_argument('--divergence_loss_weight', type=float, default=config['loss_weights']['divergence_weight'], help='Weight for divergence loss term')
...
def train():
    ...
    if atm_reshaped.ndim > 2:
        atm_reshaped = atm_reshaped.reshape(atm_reshaped.shape[0], -1)
    ...
    for epoch in range(args.epochs_1):
        ...
        for batch_idx, (batch_in, batch_out) in enumerate(train_loader):
            ...
            loss_sum = (args.loss_T_weight_1 * loss_T + 
                       args.loss_atm_weight_1 * loss_atm + 
...
if __name__ == '__main__':
    ...
    history_loss, history_metrics, history_lr, history_divergence_loss = train()

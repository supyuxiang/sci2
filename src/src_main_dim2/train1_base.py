...
# 加载配置和数据
config = load_config()
x_scaled_1, y_scaled_1, T_1, atm_1, u_1, v_1 = data_preprocessing_1()
...
parser.add_argument('--epochs_1', type=int, default=config['training_1']['epochs'])
parser.add_argument('--batch_size_1', type=int, default=config['training_1']['batch_size'])
parser.add_argument('--learning_rate_1', type=float, default=config['training_1']['learning_rate'])
parser.add_argument('--weight_decay_1', type=float, default=config['training_1']['weight_decay'])
parser.add_argument('--patience_1', type=int, default=config['training_1']['patience'])
parser.add_argument('--device_1', type=str, default=str(device))
parser.add_argument('--use_data_parallel_1', type=bool, default=config['device']['use_data_parallel'])
parser.add_argument('--gpu_device_ids_1', type=str, default=str(config['device']['gpu_device_ids']), 
                   help='GPU device IDs as string, e.g., "8" or "0,1,2"')
parser.add_argument('--early_stopping_patience_1', type=int, default=config['training_1']['early_stopping_patience'])
parser.add_argument('--loss_function_1', type=str, default=config['training_1']['loss_function'])
parser.add_argument('--scheduler_1', type=str, default=config['training_1']['scheduler'])
parser.add_argument('--hidden_size_1', type=int, default=config['model']['hidden_size'])
parser.add_argument('--num_heads_1', type=int, default=config['model']['num_heads'])
parser.add_argument('--dropout_1', type=float, default=config['model']['dropout'])
parser.add_argument('--optimizer_1', type=str, default=config['training_1']['optimizer'])
parser.add_argument('--x_scaled_1', type=np.ndarray, default=x_scaled_1)
parser.add_argument('--y_scaled_1', type=np.ndarray, default=y_scaled_1)
parser.add_argument('--T_1', type=np.ndarray, default=T_1)
parser.add_argument('--atm_1', type=np.ndarray, default=atm_1)
parser.add_argument('--u_1', type=np.ndarray, default=u_1)
parser.add_argument('--v_1', type=np.ndarray, default=v_1)
parser.add_argument('--loss_T_weight_1', type=float, default=config['loss_weights']['T_weight'])
parser.add_argument('--loss_atm_weight_1', type=float, default=config['loss_weights']['atm_weight'])
parser.add_argument('--loss_u_weight_1', type=float, default=config['loss_weights']['u_weight'])
parser.add_argument('--loss_v_weight_1', type=float, default=config['loss_weights']['v_weight'])
parser.add_argument('--model_1', type=str, default=config['model']['name_1'])
...
# 初始化SwanLab运行
run_1 = swanlab.init(
    name=f'{args.model_1}_MultiTask_Adaptive_Training_1',
    project='sci',
    config={
        "model": args.model_1,
...
def plot_data_distribution(x_scaled, y_scaled, T, atm, u, v, save_path='../results/visualizations/1'):
    ...
    axes[1, 0].hist(atm.flatten(), bins=50, alpha=0.7, color='orange', edgecolor='black')
    axes[1, 0].set_title('Atmospheric Pressure Distribution')
    axes[1, 0].set_xlabel('Atmospheric Pressure')
...
def plot_correlation_matrix(x_scaled, y_scaled, T, atm, u, v, save_path='../results/visualizations/1'):
...
# Training model
def train():
    x_scaled, y_scaled, T_reshaped, atm_reshaped, u_reshaped, v_reshaped = args.x_scaled_1, args.y_scaled_1, args.T_1, args.atm_1, args.u_1, args.v_1
    ...
    if args.model_1 == "EnhancedCARNet":
        ...
        atm_model = EnhancedCARNet(hidden_dim=args.hidden_size_1, num_heads=args.num_heads_1, dropout=args.dropout_1)
        ...
        atm_model = EnhancedCARNet_v2(hidden_dim=args.hidden_size_1, num_heads=args.num_heads_1, dropout=args.dropout_1)
    ...
    if (args.use_data_parallel_1 and torch.cuda.is_available() 
        ...
        atm_model = atm_model.to(primary_device)
        v_model = v_model.to(primary_device)
        u_model = u_model.to(primary_device)
        T3_model = T3_model.to(primary_device)
        print(f"Enabling DataParallel on GPUs: {device_ids}")
        print(f"Models moved to primary device: {primary_device}")
        atm_model = nn.DataParallel(atm_model, device_ids=device_ids)
    ...
    train_dataset = TensorDataset(torch.FloatTensor(np.concatenate((x_scaled, y_scaled), axis=1)), 
                                 torch.FloatTensor(np.concatenate((T_reshaped, atm_reshaped, u_reshaped, v_reshaped), axis=1)))
    ...
    print(f"Loss weights - T: {args.loss_T_weight_1}, atm: {args.loss_atm_weight_1}, u: {args.loss_u_weight_1}, v: {args.loss_v_weight_1}")
    print("-" * 50)
    ...
    plot_data_distribution(x_scaled, y_scaled, T_reshaped, atm_reshaped, u_reshaped, v_reshaped, f'../results/visualizations/1_{args.optimizer_1}_{args.scheduler_1}_{args.loss_function_1}')
    plot_correlation_matrix(x_scaled, y_scaled, T_reshaped, atm_reshaped, u_reshaped, v_reshaped, f'../results/visualizations/1_{args.optimizer_1}_{args.scheduler_1}_{args.loss_function_1}')
    ...
    for epoch in range(args.epochs_1):
        epoch_loss = 0
        epoch_predictions = {'T': [], 'atm': [], 'u': [], 'v': []}
        epoch_targets = {'T': [], 'atm': [], 'u': [], 'v': []}
        
        for batch_idx, (batch_in, batch_out) in enumerate(train_loader):
            # 确定目标设备
            if (args.use_data_parallel_1 and torch.cuda.is_available() 
                and len(args.gpu_device_ids_1) >= 2):
                target_device = f"cuda:{args.gpu_device_ids_1[0]}"
            else:
                target_device = args.device_1
            
            batch_in = batch_in.to(target_device)
            batch_out = batch_out.to(target_device)
            optimizer.zero_grad()
            
            # Forward pass
            outputs_T = T3_model(batch_in)
            outputs_atm = atm_model(batch_in)
            ...
            if isinstance(outputs_atm, tuple):
                outputs_atm, _ = outputs_atm
            ...
            # 添加偏移量
            outputs_atm = outputs_atm + 101325  # 凸出小变化

            loss_T = criterion(outputs_T, batch_out[:,0].unsqueeze(1))
            loss_atm = criterion(outputs_atm, batch_out[:,1].unsqueeze(1))
            loss_v = criterion(outputs_v, batch_out[:,2].unsqueeze(1))
            loss_u = criterion(outputs_u, batch_out[:,3].unsqueeze(1))

            loss_sum = args.loss_T_weight_1 * loss_T + args.loss_atm_weight_1 * loss_atm + args.loss_v_weight_1 * loss_v + args.loss_u_weight_1 * loss_u
            ...
            epoch_predictions['atm'].extend(outputs_atm.squeeze().cpu().detach().numpy())
            ...
            epoch_targets['atm'].extend(batch_out[:,1].cpu().numpy())
            ...
            if batch_idx % 5 == 0:
                ...
                print(f"Epoch {epoch+1}/{args.epochs_1}, Batch {batch_idx}, Loss: {loss_sum.item():.6f}, LR: {current_lr:.2e}")
            
            # Clean GPU memory
            del outputs_T, outputs_atm, outputs_v, outputs_u
        ...
        if (epoch + 1) % 10 == 0:
            print(f"Epoch {epoch+1}/{args.epochs_1}, Avg Loss: {avg_loss:.6f}, LR: {current_lr:.2e}")
...
if __name__ == '__main__':
    ...
    print(f"Training completed! All visualization results saved to ../results/visualizations/1_{args.optimizer_1}_{args.scheduler_1}_{args.loss_function_1}/")
    print(f"Training history saved to ../results/training_history_1_{args.optimizer_1}_{args.scheduler_1}_{args.loss_function_1}/")
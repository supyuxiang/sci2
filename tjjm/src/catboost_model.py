...
# 初始化SwanLab
run = init_swanlab("catboost_model.py")

print("="*60)
print("CatBoost Regression Temperature Prediction Analysis")
print("="*60)
...
# 模型训练
print("\nStarting CatBoost Regression model training...")
...
# 记录模型参数和指标到SwanLab
model_params = {
    'iterations': catboost_model.get_params()['iterations'],
    'learning_rate': catboost_model.get_params()['learning_rate'],
    'depth': catboost_model.get_params()['depth'],
    'random_state': catboost_model.get_params()['random_state'],
...
fig.suptitle('CatBoost Regression Temperature Prediction Analysis Results', fontsize=16, fontweight='bold')
...
axes[0].bar(importance_df['Feature'], importance_df['Importance'], color=['#ff7f0e', '#2ca02c'], alpha=0.8)
axes[0].set_title('Feature Importance', fontsize=14, fontweight='bold')
axes[0].set_ylabel('Importance Score')
axes[0].grid(True, alpha=0.3)

# 2.2 特征重要性饼图
axes[1].pie(feature_importance, labels=feature_names, autopct='%1.1f%%', startangle=90, colors=['#ff7f0e', '#2ca02c'])
...
# 多个评估指标
cv_mae = cross_val_score(catboost_model, X, y, cv=kfold, scoring='neg_mean_absolute_error')
cv_rmse = cross_val_score(catboost_model, X, y, cv=kfold, scoring='neg_root_mean_squared_error')
cv_r2 = cross_val_score(catboost_model, X, y, cv=kfold, scoring='r2')
...
print(f"\nCatBoost Regression Model Parameters:")
print(f"Number of Iterations: {catboost_model.get_params()['iterations']}")

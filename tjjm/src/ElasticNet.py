...
# 初始化SwanLab
run = init_swanlab("ElasticNet.py")

print("="*60)
print("ElasticNet Regression Temperature Prediction Analysis")
print("="*60)
...
# 模型训练
print("\nStarting ElasticNet Regression model training...")
elasticnet_model = ElasticNet(alpha=1.0, l1_ratio=0.5, fit_intercept=True, max_iter=1000)
...
model_params = {
    ...
    'fit_intercept': elasticnet_model.fit_intercept,
    'max_iter': elasticnet_model.max_iter,
    'n_features': elasticnet_model.n_features_in_,
    'coef': elasticnet_model.coef_.tolist(),
...
fig.suptitle('ElasticNet Regression Temperature Prediction Analysis Results', fontsize=16, fontweight='bold')
...
# 多个评估指标
cv_mae = cross_val_score(elasticnet_model, X, y, cv=kfold, scoring='neg_mean_absolute_error')
cv_rmse = cross_val_score(elasticnet_model, X, y, cv=kfold, scoring='neg_root_mean_squared_error')
cv_r2 = cross_val_score(elasticnet_model, X, y, cv=kfold, scoring='r2')
...
print(f"\nElasticNet Regression Model Parameters:")
print(f"Alpha: {elasticnet_model.alpha}")
print(f"L1 Ratio: {elasticnet_model.l1_ratio}")
print(f"Fit Intercept: {elasticnet_model.fit_intercept}")
print(f"Max Iterations: {elasticnet_model.max_iter}")

...
# 初始化SwanLab
run = init_swanlab("linear.py")

print("="*60)
print("Linear Regression Temperature Prediction Analysis")
print("="*60)
...
model_params = {
    ...
    'n_features': linear_model.n_features_in_,
...
fig.suptitle('Linear Regression Temperature Prediction Analysis Results', fontsize=16, fontweight='bold')
...
# 1.4 残差分布直方图
axes[1, 1].hist(residuals.flatten(), bins=30, alpha=0.7, color='#9467bd', edgecolor='black')
...
# 多个评估指标
cv_mae = cross_val_score(linear_model, X, y, cv=kfold, scoring='neg_mean_absolute_error')
cv_rmse = cross_val_score(linear_model, X, y, cv=kfold, scoring='neg_root_mean_squared_error')
cv_r2 = cross_val_score(linear_model, X, y, cv=kfold, scoring='r2')

...
results_dir = "../results/best_base_model_choose_xgb"
...
# 初始化SwanLab
run = init_swanlab("best_base_model_choose_xgb.py")

print("="*60)
print("XGBoost Best Base Model Selection Analysis")
print("="*60)
...
plt.scatter(n_values[best_idx], mae_list[best_idx], color='red', s=200, marker='*',
            label=f'Optimal Parameter: {n_values[best_idx]}\nMAE: {mae_list[best_idx]:.4f}')
...
fig.suptitle('XGBoost Best Base Model Analysis Results', fontsize=16, fontweight='bold')
...
plt.title('XGBoost Best Model Feature Importance', fontsize=16, fontweight='bold')

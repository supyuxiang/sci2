...
# 初始化SwanLab
run = init_swanlab("lasso.py")

print("="*60)
print("Lasso Regression Temperature Prediction Analysis")
print("="*60)
...
# 模型训练
print("\nStarting Lasso Regression model training...")
lasso_model = Lasso(alpha=1.0, fit_intercept=True, max_iter=1000)
...
model_params = {
    ...
    'fit_intercept': lasso_model.fit_intercept,
    'max_iter': lasso_model.max_iter,
    'n_features': lasso_model.n_features_in_,
...
fig.suptitle('Lasso Regression Temperature Prediction Analysis Results', fontsize=16, fontweight='bold')
...
# 多个评估指标
cv_mae = cross_val_score(lasso_model, X, y, cv=kfold, scoring='neg_mean_absolute_error')
cv_rmse = cross_val_score(lasso_model, X, y, cv=kfold, scoring='neg_root_mean_squared_error')
cv_r2 = cross_val_score(lasso_model, X, y, cv=kfold, scoring='r2')

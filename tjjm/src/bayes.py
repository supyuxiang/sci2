...
# 初始化SwanLab
run = init_swanlab("bayes.py")

print("="*60)
print("Bayesian Regression Temperature Prediction Analysis")
print("="*60)
...
# 模型训练
print("\nStarting Bayesian regression model training...")
...
# 记录模型参数和指标到SwanLab
model_params = {
    'alpha_1': bayesian_model.alpha_1,
    'alpha_2': bayesian_model.alpha_2,
    'lambda_1': bayesian_model.lambda_1,
    'lambda_2': bayesian_model.lambda_2,
    'fit_intercept': bayesian_model.fit_intercept,
    'n_features': bayesian_model.n_features_in_,
    'coef': bayesian_model.coef_.tolist(),
    'intercept': float(bayesian_model.intercept_)
}
...
fig.suptitle('Bayesian Regression Temperature Prediction Analysis Results', fontsize=16, fontweight='bold')
...
# 多个评估指标
cv_mae = cross_val_score(bayesian_model, X, y, cv=kfold, scoring='neg_mean_absolute_error')
cv_rmse = cross_val_score(bayesian_model, X, y, cv=kfold, scoring='neg_root_mean_squared_error')
cv_r2 = cross_val_score(bayesian_model, X, y, cv=kfold, scoring='r2')
...
print(f"alpha_1: {bayesian_model.alpha_1:.6f}")
print(f"alpha_2: {bayesian_model.alpha_2:.6f}")
print(f"lambda_1: {bayesian_model.lambda_1:.6f}")
print(f"lambda_2: {bayesian_model.lambda_2:.6f}")
